...
impl MinecraftAuthenticator {
	...
	pub async fn initial_auth(&self, callback: fn(code: String, uri: String, message: String)) -> Result<Account, AuthenticationError> {
		...
		loop {
			...
			let grant_response = self.reqwest_client
			...
			match grant_response.status().as_u16() {
				200 => {
					...
					return self.authenticate(poll_success.access_token, poll_success.refresh_token).await
				}
				_ => {
	...
	pub async fn refresh(&self, account: Account) -> Result<Account, AuthenticationError> {
		let grant_response: GrantSuccessResponse = self.reqwest_client
			.post("https://login.microsoftonline.com/consumers/oauth2/v2.0/token")
			.form(&vec![
				("client_id", self.client_id.as_str()),
		...
		return self.authenticate(grant_response.access_token, grant_response.refresh_token).await
	}

	async fn authenticate(&self, access_token: String, refresh_token: String) -> Result<Account, AuthenticationError> {
